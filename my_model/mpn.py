...
from func.myloss import my_CE_EM, my_CrossEntropyLoss_KL_MSE
...
class GaussianNPNLinearLayer(nn.Module):
    def __init__(self, input_features, output_features):
        ...
        if torch.isnan(self.W_m).any() or torch.isnan(self.M_s).any():
            print('Wrong in create the NPN LinearLayer')
            print(f'{self.W_m},{self.M_s},{input_features},{output_features}')
        self.b_m, self.p_s = self.get_init_b(output_features)
        if torch.isnan(self.b_m).any() or torch.isnan(self.p_s).any():
            print('Wrong in create the NPN LinearLayer')
            print(f'{self.b_m},{self.p_s},{input_features},{output_features}')
...
class GaussianNPN(nn.Module):
    def __init__(self, input_features, cardinalities, node_type, hidden_sizes, para):
        # input_features = sum_cardinalities
        # cardinalities = output_features
        ...
        if self.node_type == 'D':
            self.lossfn = my_CE_EM(reduction='sum', para=para)
        elif self.node_type == 'C':
